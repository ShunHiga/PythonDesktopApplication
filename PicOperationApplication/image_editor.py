...
def show_image_editor(image_path):
	...
	def_img = raw_img.resize((400,400))
	#レイアウト左側を定義
	col_left = [
		[sg.Image(convert_png(def_img), key="image")]
	]
	#レイアウト右側を定義
	col_right = [
		[sg.Text("コントラスト")],
		[sg.Slider(key="contrast",range=(0,2), resolution=0.1, default_value=1,
			 orientation="h", enable_events=True)],
		[sg.Text("明るさ")],
		[sg.Slider(key="brightness", range=(0,10), resolution=0.1, default_value=1, orientation="h", enable_events=True)],
		[sg.Text("ぼかし")],
		[sg.Slider(key="blur", range=(0,10), resolution=0.1, default_value=0, orientation="h", enable_events=True)]
	]
	...
	while True:
		...
		if event in (sg.WIN_CLOSED, "閉じる"):
			break
		if event == "contrast" or event == "brightness" or event == "blur":
			#現在の設定でフィルターをかける
